...
Backed by Supabase (`js/leaderboard.js`): public REST + anon key, client-side. **One table per game**, chosen by `GAME.leaderboard.table` — `scores` for Bogotá, `jungle_scores` for Kong Run. Separate tables rather than a `game` column so the boards are genuinely independent: each shows its own top N, and a schema change to one cannot disturb the other. The schema lives in `supabase/leaderboard.sql` so the shape the client depends on is not knowledge stored only in a dashboard. **Data integrity comes from the `scores` table's own CHECK constraints** (score range, name length) — the RLS insert policy itself permits any row shape, so don't assume RLS alone bounds submitted data; add a matching CHECK constraint for any new submitted field. `fetchLeaderboard`/`submitScoreToLeaderboard` are thin `fetch()` wrappers; failures degrade gracefully to a "couldn't reach leaderboard" message rather than blocking gameplay. Name is uppercased, capped at 12 chars, remembered in `localStorage` for next run. Rendered rows coerce `score` to a number and escape `player_name` before touching `innerHTML`.
...
**Sharing a score uses the platform, not a library.** The game-over screen carries a secondary `↗ Share score` next to TRY AGAIN — outlined and unfilled so it invites without competing with the primary CTA. `navigator.share()` is the whole feature on mobile, since the OS sheet is where links actually get shared; the clipboard is the desktop fallback, and the button reports which happened. Dismissing the share sheet rejects the promise with `AbortError`, which is a choice rather than a failure and is the one rejection swallowed on purpose. The shared URL is rebuilt from `origin + pathname` so a forced `?theme=` never travels with it.
...
**Asset paths resolve against the project root, not the page.** A pack writes `"sprites/x.webp"`, which is what a page at the root needs — but the jungle lives at `/jungle/`, and GitHub Pages serves the whole site under `/ragnar-runner/`. A bare relative path breaks on the first, an absolute one on the second. `js/paths.js` uses `import.meta.url` as the fixed point: that module is known to be `<root>/js/paths.js` on every host and from every page, so the root is one level up from it, and `asset()` resolves everything through that. Anything a game names — spritesheet, backgrounds, obstacle pictures, audio, hero shots, its strings file — goes through it.

**A page declares its game.** `<html data-game="jungle">`, read by `js/active-game.js`, which imports both packs statically and picks one. Static because the engine reads `GAME` during module evaluation: a dynamic `import()` would make it a promise and force every consumer to become async, for a saving of a few KB. Unknown or missing values fall back to Bogotá — a typo in a URL should still be playable.

The jungle gets its own HTML file rather than a `?game=` parameter because a link preview needs its own `<title>`, description and `og:image`, and a static host serves one file for every query string. The cost is a duplicated `<body>`; the copy inside it is not duplicated, since every word comes from the pack's `strings.json`.
