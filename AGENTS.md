...
- The engine (everything directly under `js/`) must never name a sprite, a sound file, a colour, or a line of copy — all of that comes from the active game pack (`js/games/<id>/`, selected by `js/active-game.js`). New game-specific values go in the pack and are surfaced through `js/config.js`; the dependency only ever points engine -> game, never back.
- Run `python3 check.py` before pushing; it is also enforced in CI. Add a check to it whenever you find another failure that does not announce itself.
- `index.html` and `jungle/index.html` share the same `<body>`; a markup change to one has to be mirrored in the other. Wording never does — it comes from each pack's `strings.json`.
- Any path a game pack names (sprites, backgrounds, audio, obstacle pictures, its strings file) must be resolved through `asset()` in `js/paths.js`. A bare relative path breaks the page at `/jungle/` and an absolute one breaks GitHub Pages' `/ragnar-runner/` prefix.
- Never hardcode a user-visible string in a `.js` file or in `index.html`. All copy belongs in the active game's `strings.json`, referenced by `data-text="key"` in markup and `t('key', vars)` in code. The `<title>` and the `<meta>`/Open Graph description are the only exceptions, because crawlers read them before scripts run.
- Seasonal looks have exactly two homes: the palette in `css/style.css` (`:root[data-theme="..."]`) and the canvas layer in `js/seasonal.js`. `js/theme.js` owns the month mapping and is the only thing that reads the clock. Never branch on the theme name anywhere else, and never let a theme change layout — only colours and the overlay.
